...
/datum/element/fulp_snailcrawl/Detach(mob/living/carbon/target)
	. = ..()
	UnregisterSignal(target, COMSIG_MOVABLE_MOVED)
	if(istype(target))
		target.remove_movespeed_modifier(/datum/movespeed_modifier/snail_crawl)

/datum/element/fulp_snailcrawl/proc/snail_crawl(mob/living/carbon/snail)
	SIGNAL_HANDLER
...
/datum/element/fulp_snailcrawl/proc/lubricate(atom/movable/snail)
	SIGNAL_HANDLER

	var/turf/open/OT = get_turf(snail)
	if(istype(OT))
		OT.MakeSlippery(TURF_WET_WATER, 3)
		return TRUE
