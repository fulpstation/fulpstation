...
/datum/objective/assassinate/internal/find_target(dupe_search_range, list/blacklist)
	return
...
/datum/objective/assassinate/internal/update_explanation_text()
	if(target && target.current && (target.current.stat != DEAD))
		...
		explanation_text = "Turn in the corpse of [target.name], who has been obliterated, to the Devil using a calling card (Alt Click a paper)."
