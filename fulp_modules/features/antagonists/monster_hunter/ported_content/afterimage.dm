...
/datum/component/after_image/proc/sync_after_images(dir_override=null)
	...
	if(color_cycle)
		...
		targeted_image.color = COLOR_RED
...
/obj/effect/after_image/proc/sync_with_parent(mob/living/parent, loc_override = null, actual_loc = TRUE, dir_override = null)
	if(!src.active)
		return
	...
	if(src.appearance_ref != parent_appearance_ref)
		...
		src.plane = initial(src.plane) - 0.01
	...
	// (This portion of the code is a rough "patch" for a visual bug inherent to the original, ported feature.)
	// For now we try to turn invisible if ANY visual transformation is applied to us.
	// The alternative is effect misalignment.
	// (Lying)
