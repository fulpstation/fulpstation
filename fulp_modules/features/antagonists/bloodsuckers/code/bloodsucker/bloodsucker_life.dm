...
/datum/antagonist/bloodsucker/proc/HandleHealing(mult = 1)
	...
	var/bruteheal = min(user.get_brute_loss_non_prosthetic(), actual_regen) // BRUTE: Always Heal
	...
	if(amInCoffin && HAS_TRAIT(user, TRAIT_NODEATH))
		if(HAS_TRAIT(owner.current, TRAIT_MASQUERADE) && (COOLDOWN_FINISHED(src, bloodsucker_spam_healing)))
			...
			return
		fireheal = min(user.get_fire_loss_non_prosthetic(), actual_regen)
	...
	// In Torpor, but not in a Coffin? Heal faster anyways.
		...
		fireheal = min(user.get_fire_loss_non_prosthetic(), actual_regen) / 1.2 // 20% slower than being in a coffin
		mult *= 3
	// Heal if Damaged
	if((bruteheal + fireheal > 0) && mult != 0) // Just a check? Don't heal/spend, and return.
		// We have damage. Let's heal (one time)
		user.adjust_brute_loss(-bruteheal * mult, forced=TRUE) // Heal BRUTE / BURN in random portions throughout the body.
		user.adjust_fire_loss(-fireheal * mult, forced=TRUE)
