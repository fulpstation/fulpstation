...
/datum/antagonist/bloodsucker/proc/break_masquerade()
	...
	for(var/datum/mind/clan_minds as anything in get_antag_minds(/datum/antagonist/bloodsucker))
		...
		if(!isliving(clan_minds.current))
			continue
		to_chat(clan_minds, span_userdanger("[owner.current], the [vassal_color]-colored bloodsucker, has broken the Masquerade! Ensure [owner.current.p_they()] [owner.current.p_are()] eliminated at all costs!"))
