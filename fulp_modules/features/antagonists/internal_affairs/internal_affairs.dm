...
/datum/antagonist/traitor/internal_affairs/pick_employer(faction)
	var/iaa_faction = prob(EXTERNAL_CHANCE) ? ROLE_EXTERNAL_AFFAIRS : ROLE_INTERNAL_AFFAIRS

	employer = iaa_faction

	traitor_flavor = strings(IAA_FLAVOR_FILE, employer, directory = "fulp_modules/strings")
...
/datum/antagonist/traitor/internal_affairs/proc/forge_single_generic_objective()
	if(prob(KILL_PROB))
