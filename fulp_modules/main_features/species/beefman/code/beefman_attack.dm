...
/datum/species/beefman/disarm(mob/living/carbon/human/user, mob/living/carbon/human/target, datum/martial_art/attacker_style)
	...
	if (user == target)
		...
		var/obj/item/bodypart/selected_bodypart = user.get_bodypart(check_zone(target_zone)) //stabbing yourself always hits the right target
		...
		if ((target_zone in allowedList) && selected_bodypart)
			...
			// Pry it off...
			user.visible_message("[user] grabs onto [p_their()] own [selected_bodypart.name] and pulls.", span_notice("You grab hold of your [selected_bodypart.name] and yank hard."))
			if (!do_mob(user,target))
				return TRUE

			user.visible_message("[user]'s [selected_bodypart.name] comes right off in their hand.", span_notice("Your [selected_bodypart.name] pops right off."))
			...
			user.put_in_hands(meat) // This has no way of failing because we just passed all earlier checks.
...
/datum/species/beefman/proc/handle_limb_mashing(mob/living/meat_masher, mob/living/carbon/human/beefman, /obj/item/food/meat/slab/meat)
	SIGNAL_HANDLER
