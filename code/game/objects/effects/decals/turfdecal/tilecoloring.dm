...
/obj/effect/turf_decal/tile/Initialize(mapload)
	if(SSevents.holidays)
		...
		else if (SSevents.get_holiday(PRIDE_WEEK))
			var/datum/holiday/pride_week/pride_week = SSevents.get_holiday(PRIDE_WEEK)
...
/obj/effect/turf_decal/trimline/Initialize(mapload)
	if(SSevents.holidays && SSevents.get_holiday(APRIL_FOOLS))
		color = "#[random_short_color()]"
	. = ..()
