...
/obj/machinery/autolathe/Initialize()
	AddComponent(/datum/component/material_container, list(MAT_METAL, MAT_GLASS), 0, TRUE, null, null, CALLBACK(src, PROC_REF(AfterMaterialInsert)))
	. = ..()
	...
	if (!busy)
		...
		if(href_list["make"])
			...
			if((materials.amount(MAT_METAL) >= metal_cost*multiplier*coeff) && (materials.amount(MAT_GLASS) >= glass_cost*multiplier*coeff))
				...
				addtimer(CALLBACK(src, PROC_REF(make_item), power, metal_cost, glass_cost, multiplier, coeff, is_stack), time)
