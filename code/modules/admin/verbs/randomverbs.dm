	...
	switch(punishment)
		...
		if(ADMIN_PUNISHMENT_NUGGET)
			...
			for(var/obj/item/bodypart/thing in C.bodyparts)
				...
				addtimer(CALLBACK(GLOBAL_PROC, GLOBAL_PROC_REF(playsound), C, 'sound/effects/cartoon_pop.ogg', 70), timer)
				addtimer(CALLBACK(C, TYPE_PROC_REF(/mob/living, spin), 4, 1), timer - 0.4 SECONDS)
				timer += 2 SECONDS
