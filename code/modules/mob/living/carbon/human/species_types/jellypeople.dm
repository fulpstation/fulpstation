...
/datum/species/jelly/spec_life(mob/living/carbon/human/H)
	if(H.stat == DEAD || HAS_TRAIT(H, TRAIT_NONATURALHEAL)) // Fulpstation Bloodsuckers edit: Can't farm slime jelly from a dead slime/jelly + No regeneration!
		return
	if(!H.blood_volume)
		H.blood_volume += 5
		H.adjustBruteLoss(5)
...
/datum/species/jelly/slime/spec_life(mob/living/carbon/human/H)
	if((HAS_TRAIT(H, TRAIT_NONATURALHEAL))) // Fulpstation Bloodsuckers edit
		return
