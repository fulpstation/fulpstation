...
/mob/living/simple_animal/bot/secbot/proc/retaliate(mob/living/carbon/human/H)
	...
	threatlevel = H.assess_threat_fulp(judgement_criteria, src, TRUE, null, weaponcheck=CALLBACK(src, PROC_REF(check_for_weapons))) //FULPSTATION SECBOT FEEDBACK PR -Surrealistik Feb 2020
	threatlevel += 6
	if(threatlevel >= 4)
		target = H
		mode = BOT_HUNT
...
/mob/living/simple_animal/bot/secbot/proc/stun_attack(mob/living/carbon/C, harm = FALSE)
	...
	if(ishuman(C))
		...
		threat = H.assess_threat_fulp(judgement_criteria, src, FALSE, null, weaponcheck=CALLBACK(src, PROC_REF(check_for_weapons))) //FULPSTATION SECBOT FEEDBACK PR -Surrealistik Feb 2020
	else
		C.Paralyze(100)
		C.stuttering = 5
