...
/datum/preferences/proc/ShowChoices(mob/user)
	...
	switch(current_tab)
		if (0) // Character Settings#
			...
			if((EYECOLOR in pref_species.species_traits)) // && !(NOEYESPRITES in pref_species.species_traits)) [FULP]  If we want Eye Color, let it show eye color. Maybe we want eye color to affect something else? (Beef Eyes) && !(NOEYESPRITES in pref_species.species_traits))
