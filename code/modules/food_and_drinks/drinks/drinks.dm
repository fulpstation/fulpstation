...
/obj/item/reagent_containers/food/drinks/afterattack(obj/target, mob/user , proximity)
	...
	if(target.is_refillable() && is_drainable()) //Something like a glass. Player probably wants to transfer TO it.
		...
		if(iscyborg(user)) //Cyborg modules that include drinks automatically refill themselves, but drain the borg's cell
			...
			bro.cell.use(30)
			addtimer(CALLBACK(reagents, TYPE_PROC_REF(/datum/reagents, add_reagent), refill, trans), 600)*/ //FULPSTATION FIXES by Surrealistik April 2020 This is a half-assed feature often mistaken as a bug that is utterly broken as hell. Good riddance.
