...
/obj/structure/closet/supplypod/Initialize(mapload, var/turf/spawn_location)
	. = ..()
	if (!spawn_location)
...
/obj/structure/closet/supplypod/proc/preOpen() //Called before the open_pod() proc. Handles anything that occurs right as the pod lands.
	...
	if (style == STYLE_GONDOLA) //Checks if we are supposed to be a gondola pod. If so, create a gondolapod mob, and move this pod to nullspace. I'd like to give a shout out, to my man oranges
		...
		moveToNullspace()
		addtimer(CALLBACK(src, PROC_REF(open_pod), benis), openingDelay) //After the openingDelay passes, we use the open proc from this supplyprod while referencing the contents of the "holder", in this case the gondolapod mob
	else if (style == STYLE_SEETHROUGH)
		open_pod(src)
	else
		addtimer(CALLBACK(src, PROC_REF(open_pod), src), openingDelay) //After the openingDelay passes, we use the open proc from this supplypod, while referencing this supplypod's contents
...
/obj/structure/closet/supplypod/proc/open_pod(atom/movable/holder, broken = FALSE, forced = FALSE) //The holder var represents an atom whose contents we will be working with
	...
	if (style == STYLE_SEETHROUGH)
		...
		if (reversing)
			addtimer(CALLBACK(src, PROC_REF(SetReverseIcon)), departureDelay/2) //Finish up the pod's duties after a certain amount of time
		if(!stay_after_drop) // Departing should be handled manually
			addtimer(CALLBACK(src, PROC_REF(startExitSequence), holder), departureDelay) //Finish up the pod's duties after a certain amount of time
...
/obj/structure/closet/supplypod/close(atom/movable/holder) //Closes the supplypod and sends it back to centcom. Should only ever be called if the "reversing" variable is true
	...
	holder.setClosed()
	addtimer(CALLBACK(src, PROC_REF(preReturn), holder), 10) //Start to leave a second after closing for cinematic effect
...
/obj/effect/pod_landingzone/proc/setupSmoke(rotation)
	...
	for ( var/i in 1 to length(smoke_effects))
		...
		addtimer(CALLBACK(S, TYPE_PROC_REF(/obj/effect/supplypod_smoke, drawSelf), i), time, TIMER_CLIENT_TIME) //Go onto the last step after a very short falling animation
